...
### One ID 흐름 스킨 (메인 디자인)
계정 전환 흐름은 **실서비스(One ID) 스타일을 메인**으로 하고, 본 디자인 시스템 토큰으로 구현한다
(Pleos Connect 시스템은 서브 — 토큰·컴포넌트 기반 제공).
- 화이트 배경(`--surface`) · **블랙 필 버튼**(`--basic-900` / 텍스트 `--basic-00`) · 아웃라인 보조 버튼.
- 라운드는 `--r-sm`(8px) 기준. 중앙 정렬 타이틀/설명, 상단 중앙 브랜드 헤더.
- 단계 표시는 **숫자 도트 인디케이터**(현재 = 숫자 블랙 서클, 지난 단계 = 블랙 도트).
- 파랑(`--info`)은 링크·안내 박스(`--primary-weak` 배경)·강조에만 사용.
