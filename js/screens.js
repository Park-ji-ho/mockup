/* =========================================================
   ...
   메인 디자인: 실서비스 스타일. 디자인 시스템 토큰으로 구현.
...
(function () {
  ...
  function brand() {
    return (
      '<header class="uni__brand"><b>**</b>' +
      '<span><em>**</em> Account</span></header>'
    );
  }
  ...
  function dots(step) {
    ...
    return '<div class="uni__dots"><span class="indicator indicator--wide">' + out + "</span></div>";
  }

  function overlay(state) {
    return state.loading
      ? '<div class="uni__overlay"><span class="loading loading--tension"><span class="spinner"></span></span></div>'
      : "";
  }
  ...
  function toastLayer(state) {
    ...
    return (
      '<div class="uni__toast"><div class="toast' +
  ...
  function shell(step, state, main, footer) {
    ...
    return (
      '<div class="uni">' +
      brand() +
      '<div class="uni__scroll">' +
      dots(step) +
      '<div class="uni__content' + anim + '">' + main + "</div>" +
      "</div>" +
      (footer ? '<div class="uni__footer' + anim + '">' + footer + "</div>" : "") +
      toastLayer(state) +
      overlay(state) +
      "</div>"
    );
  ...
  function step1(state) {
    var main =
      '<h2 class="uni__title">하나의 계정으로,<br />모든 그룹 서비스</h2>' +
      '<div class="uni__links"><span data-action="info">** 계정 알아보기</span><i>|</i><span data-action="info">** 계정 전환 방법</span></div>' +
      '<div class="uni__logo"><em>**</em></div>' +
      '<div class="uni__notice uni__notice--fill">2027.1.25까지 미전환 시<br />기존 계정 로그인 불가</div>' +
      '<p class="uni__caption">혜택과 포인트는 그대로 유지됩니다</p>';
    var footer =
  ...
  function step2(state) {
    ...
    var main =
      '<h2 class="uni__title">이메일 인증</h2>' +
      '<p class="uni__desc">** 계정에 사용할<br />이메일을 인증해 주세요</p>' +
      '<div class="uni__field uni__inline">' +
      '<span class="tfield">' +
      ...
      "</span>" +
      '<button class="btn btn--outline uni__send" data-action="send">전송</button>' +
      "</div>" +
      '<div class="uni__field uni__numrow">' +
      ...
      "</div>" +
      '<span class="uni__timer">' + fmtTimer(state.timerSec) + "</span>" +
      "</div>" +
      (state.error
        ? '<p class="tfield__helper">인증번호가 올바르지 않습니다. 다시 확인해 주세요.</p>'
        : "") +
      '<p class="uni__caption uni__caption--left">인증번호 전송은 하루 5회까지 가능합니다</p>' +
      '<div class="uni__notice">✓ 인증한 이메일이<br /><b>** 계정 ID가 됩니다</b></div>' +
      '<p class="uni__caption" data-action="info">개인정보 처리 안내 보기 →</p>';
    var footer =
  ...
  function step3(state) {
    function acct(mark, name, sub) {
      return (
        '<div class="uni__acct">' +
        '<span class="uni__mark">' + mark + "</span>" +
        '<span class="uni__acct-meta"><span class="uni__acct-name">' + name + "</span>" +
        '<span class="uni__acct-sub">' + sub + "</span></span>" +
        '<span class="uni__check">✓</span>' +
        "</div>"
      );
    }
    var main =
      '<h2 class="uni__title">박지호님의 계정<br />3개를 찾았어요</h2>' +
      '<div class="uni__acct-list">' +
      acct("A", "** 서비스 A", "포인트 32,000P") +
      acct("B", "** 앱 B", "구독 1건") +
      acct("C", "** 서비스 C", "이용 이력") +
      "</div>" +
      '<p class="uni__caption">인증 이메일 기준 보유 계정 자동 조회</p>';
    var footer =
  ...
  function step4(state) {
    function row(key, tag, label, checked) {
      var warn = state.warn && tag === "필수" && !checked ? " uni__consent--warn" : "";
      return (
        '<div class="uni__consent' + warn + '" data-consent="' + key + '">' +
        '<span class="uni__consent-label">[' + tag + "] " + label + "</span>" +
    ...
    var main =
      '<h2 class="uni__title uni__title--left">약관 및 동의</h2>' +
      '<div class="uni__consent-list">' +
  ...
  function step5(state) {
    var main =
      '<div class="uni__done-circle">✓</div>' +
      '<h2 class="uni__title">전환 완료!</h2>' +
      '<p class="uni__desc">자산 3종 승계 완료<br />모든 서비스 바로 이용</p>';
    var footer =
      '<div class="uni__bubble">아직 전환 안 한 가족이나 친구가 있나요?<br />2027.1.25 전에 함께 전환해 보세요</div>' +
