...
├── dd.html               # 다음 DD 에이전트 쇼케이스 (9단계 + 가이드 투어)
│                         #   홈→선톡→대화→실행→연동→발송→음성→루틴 제안→작업 목록
│                         #   ?preview=1 풀스크린 · ?guide=1 스포트라이트 투어
├── showcase.html         # ** 계정 전환 쇼케이스 (비공개 — 진입 카드 잠금)
...
│   ├── showcase.css      # 쇼케이스 셸(공유) + uni 흐름 @container
│   └── dd.css            # dd 흐름 @container (DESIGN §5 dd 스킨)
├── js/                   # screens/showcase + dd-screens/dd-showcase + components/main
├── README.md
